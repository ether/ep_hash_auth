# Contributing to ep_hash_auth

Thanks for helping improve `ep_hash_auth` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_hash_auth` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_hash_auth from your local clone
pnpm run plugins i --path ../ep_hash_auth

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_hash_auth/` are picked up after a server restart.
...
* Etherpad version, ep_hash_auth version, browser + OS
* Steps to reproduce
