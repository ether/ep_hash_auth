...
`ep_hash_auth` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_hash_auth`:
...
* Server: `authenticate`, `handleMessage`, `expressCreateServer`, `eejsBlock_userlist`, `eejsBlock_scripts`
